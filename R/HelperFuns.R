...
.findPerm2 <- function(phi_list, anchor_id){
  target_pi <- phi_list[[1]][,grep(anchor_id, colnames(phi_list[[1]]))]
  perms <- lapply(phi_list,
                  function(phi){
                    x_phi <- phi[,grep(anchor_id, colnames(phi))]
                    cost_mat <- x_phi %*% target_pi
                    perm <- clue::solve_LSAP(t(cost_mat), TRUE)
                    return(as.matrix(as(c(perm), "pMatrix")))
                  })
  return(perms)
}
