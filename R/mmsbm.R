...
mmsbm <- function(formula.dyad,
  ...
  if(ctrl$hessian){
   ...
   all_phi1 <- split.data.frame((t(fit[["SenderPhi"]])), # for calculating Hessian using LB
                              c(nt_id[,1]))
   all_phi2 <- split.data.frame((t(fit[["ReceiverPhi"]])),# for calculating Hessian using LB
                               c(nt_id[,2]))
    ...
    fit$hess_calc_data1<-list(all_phi1,fit[["MonadCoef1"]], ctrl$se_sim, n.blocks[1],
                                 n.hmmstates, fit[["TotNodes1"]], periods,
                                 ctrl$mu_beta1, ctrl$var_beta1, fit[["Kappa"]], t_id_n1, X1,tot_nodeid_1)                            
    cat("X1_sd: ", X1_sd,".\n")
    cat("X2_sd: ", X2_sd,".\n")                      
