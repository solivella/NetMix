#' Plot a bipartite web -- adjusted from Bipartite package with a \code{sociomatrix} input.
#'
#' The function provides a variety of plotting options for a fitted \code{mmsbm} object.
#'
#' @param web An object of class \code{data.frame}, rows are Family 1, columns Family 2.
#' 
#'


### Bipartite Plot Adjustment from Bipartite package
plotwebB <-
  function(web, method = "cca", empty = TRUE, labsize = 1, ybig = 1,
           ...
           party_color_info=NULL,bills_color_info=NULL)
  {
    ...
    if (abuns.type %in% c('additional','none')){
      ...
      for (p in 1:sum(web > 0)) {
        ...
        #i'm removing the below line so the border color is matched with icol
        ...
        polygon(c(x1+high.xoff, x2+high.xoff, x4+low.xoff, x3+low.xoff), c(y1, y2, y4, y3), col = icol, border=icol)#bicol)
      }
      #par(op)
    }
