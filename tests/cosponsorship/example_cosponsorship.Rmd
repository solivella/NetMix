...
senate<-readRDS("tests/cosponsorship/cosponsor_data/senate-107.rds")
...
d<-cbind(subset(senate,select=c(labels),subset=party=="Democrat"),subset(senate,select=c(labels),subset=party=="Republican"))
names(d)<-c("Democrat","Republican")
rownames(d)<-NULL
 kbl(d, format="latex", booktabs = T, linesep = "") %>% 
   kable_styling(font_size=5,full_width=F, position="center",latex_options =c("scale_down")) %>%# c("striped","scale_down")) %>%
...
#senators
#linesep = if (booktabs) c('', '', '', '', '\\addlinespace') else '\\hline'
d<-cbind(c(senate$labels[which(senate$party=="Democrat")],rep("",3)),subset(senate,select=c(labels),subset=party=="Republican"))
names(d)<-c("Democrat","Republican")
rownames(d)<-NULL
 kbl(d, format="latex", booktabs = T, linesep = "", caption="108th Congressional Session Senators") %>% 
   kable_styling(font_size=3,full_width=F, position="center",latex_options =c("scale_down")) %>%# c("striped","scale_down")) %>%
  footnote(general = "",threeparttable = T)
 
#bills
kbl(table(bills$billsponsor_party,bills$billtype),format="latex", booktabs = T, linesep = "", caption="108th Congressional Session Bills") %>% 
  kable_styling(font_size=3,full_width=F, position="center",latex_options =c("scale_down")) %>%
  footnote(general = "SC stands for concurrent resolutions, SE for resolutions, SJ for joint resolutions, SN for bills, and SP for amendments.",threeparttable = T)
```
