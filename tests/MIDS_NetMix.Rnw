...
\noindent We apply the model to a dynamic network of countries linked by interstate conflict.  Nodes are the 216 countries that are present in the 1816-2010 period.  Nodes enter and exit the network as countries move in and out of the international system.  Edges represent the onset of a militarized dispute between countries in a given year.  We measure edges using version 4.1 of the undirected Militarized Interstate Dispute (MID) dataset.  The dependent variable comprises 842,685 country dyad-years.
\\

\noindent We include four monadic covariates: a measure of countries' regime type (polity scores from the Polity IV dataset); their military capability (logged CINC scores from the Correlates of War National Material Capabilities dataset); and an indicator for membership in the two major alliance systems of the bipolar era, NATO and the Warsaw Pact. Dyadic covariates include an indicator for the presence of any alliance between countries, the logged geographic distance between countries, an indicator for contiguity, and a count of joint IGO memberships.  

\bigskip

\noindent The mmsbm() function fits the model and returns the estimated monadic and dyadic coefficients, blockmodel of edge formation probabilities, cluster membership probabilities for each node-year, latent state probabilities for each year, and matrix of transition probabilities between latent states.  
...
library(NetMix)  
set.seed(999)
fit_onset <- mmsbm(formula.dyad = MID_onset_DY ~ IGOmems_joint + 
                           ally + contiguity + dist,
                         formula.monad =  ~ polity + logNMC + WP + NATO, 
...
<<2, eval=TRUE, echo=FALSE, comment=NA, message=FALSE, warning=FALSE, tidy=TRUE>>=
load("tests/fit_onset.Rdata")
@

<<3, eval=TRUE, echo=TRUE, comment=NA, message=FALSE, warning=FALSE, tidy=TRUE>>=
summary(fit_onset)

head(fit_onset) 
head(fit_onset, node=FALSE) 

head(fit_onset, t=1950:1990) 
# During CW, 4=NATO, 2=Warsaw Pact, 3=neutrals, 1&5=3rd parties

fit_onset$TransitionKernel
@

<<4, eval=TRUE, echo=TRUE, comment=NA, message=FALSE, warning=FALSE, tidy=TRUE>>=
plot(fit_onset)

cluster.time(MID_onset_5_2onb) # should work with plot(fit_onset, type="membership")
@


<<5, eval=TRUE, echo=TRUE, comment=NA, message=FALSE, warning=FALSE, tidy=TRUE>>=
mean(fit_onset$dyadic.data$MID_onset_DY)
mean(predict(fit_onset, type="response"))
mean(predict(fit_onset, type="response", parametric_mm=TRUE)) # why so low? affects estimated covariate effects
...
\noindent We compare our results with a standard logit model.  To do so, we need to transform the monadic covariates to match the dyadic structure required in logit.
...
MID_dyad2$polity_2[is.na(MID_dyad2$polity_1)] <- 0
...
MID_dyad2b <- MID_dyad[MID_dyad$year <= 2010,]


## need to account for NATO/WP here as well
...
cor(coef(logit.MID)[2:5], fit_onset$DyadCoef[1:4]) # almost perfectly correlated
@

<<13d, eval=F, echo=F, comment=NA, message=FALSE, warning=FALSE, tidy=FALSE>>=
pred.logit <- predict(logit.MID, newdata = MID_dyad2b)
...
set.seed(9999)
fit_onset2008 <- mmsbm(formula.dyad = MID_onset_DY ~ IGOmems_joint + 
                           ally + contiguity + dist,
                         formula.monad =  ~ polity + logNMC + WP + NATO, 
...
logit2008 <- logit.MID <- glm(MID_onset_DY ~ IGOmems_joint + ally + contiguity + dist + 
                   dem + aut + cinc_ratio + 
                   #IGOmems_joint_missing + dist_missing +  
                   polity_missing,
