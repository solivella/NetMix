...
```{r,fig.align='center', fig.height=3.5, fig.width=5.5}
set.seed(831213)
true_beta <- array(c( -1.5, 0.5, 1.6, 0,
                     -1.5, -0.5, 1.95, -0.95),
                   c(4, 2, 1))
net2_list <-  replicate(1, 
                        NetSim(BLK = 2
                               ,NODE = 300
                               ,STATE = 1
                               ,TIME = 1 
                               ...
                               ,B_t = matrix(c(5, rep(-5, 2), 5), 
                                             ncol=2)
                               ,beta_arr = true_beta
                               ,gamma_vec = c(1.5, -0.5, 0.5)),
                        simplify = FALSE)
#vcd::ternaryplot(net2_list[[1]]$pi_vecs[[1]], col=rgb(1,0,0,0.7), dimnames = 1:3, cex=0.7,
 #                main = "Mixed-membership\nNetwork I")
```
...
set.seed(831213)
system.time(net2.models <- lapply(1,
                      function(i){res <- mmsbm(formula.dyad = Y ~ V1 + V2 + V3,
...
matplot(net2_list[[1]]$pi_vecs[[1]],t(net2.models[[1]]$res$MixedMembership[net2.models[[1]]$ord,]))
```
```{r,fig.align='center', fig.height=3, fig.width=4.5, warning=FALSE, message=FALSE}
est_fx <- lapply(1:50,
                 function(i){
                   x <- net2.models[[i]]
                   data.frame(var = rep(c("V0","V1", "V2", "V3"), 3),
                              grp = rep(c("1","2","3"), each = 4),
                              val = c(x$res$MonadCoef[,x$ord,1]),
                              iter = i)
                 }) 
...
ggplot(est_fx_df, aes(x=grp, y=val)) +
  ...
  geom_point(data = data.frame(
    var = rep(c("V0","V1", "V2", "V3"), 3),
    x=rep(c("1","2","3"), each = 4),
    y = c(true_beta[,,1])),
             aes(x=x, y=y),
             color = "red",
             cex = 2.5) +
  scale_y_continuous(limits = c(min(true_beta[,,1]),max(true_beta[,,1])))+
  theme_bw() + 
