...
N_REP <- 1
true_beta <- array(c(1, rnorm(3, 0, 3),
                     1, rnorm(3, 0, 3),
                     1, rnorm(3, 0, 3)),
                   c(4, 3, 1))
...
#beta_rand <- rnorm(N_MONAD_PRED*N_BLK)
    ...
    (bfgs_res <- optim(c(net2_list[[1]]$beta_arr[,,1]),
                       # c(lm.fit(net2_list[[1]]$X[[1]], log(t(C_t/(N*2))))$coef), 
                       alphaLB,
                       gr = alphaGr,
                       N_STATE = N_STATE,
                       N_PAR = N_MONAD_PRED * N_BLK * N_STATE,
                       ...
                       N_NODE = N,
                       N_TIME = 1, 
                       #x_t_r = t(cbind(1,scale(net2_list[[1]]$X[[1]][, -1]))),
                       x_t_r = t(net2_list[[1]]$X[[1]]),
                       e_c_t = C_t,
                       time_id_node = rep(0, N),
                       kappa_t_r = matrix(1, ncol = 1, nrow = 1),
    ...
    diffs[i,] <- ((net2_list[[1]]$beta_arr[,,1]) - array(bfgs_res$par, c(4, 3)))
  }
